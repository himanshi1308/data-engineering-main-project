...
# MAGIC %md
# MAGIC # Consolidated Pipeline Execution
# MAGIC Executes Bronze, Silver, and Gold pipeline processing sequentially in a single notebook to measure runtime performance without platform orchestration overhead.
...
from pyspark.sql.functions import (
    col, regexp_extract, current_timestamp, to_timestamp, 
    max as spark_max, broadcast, expr, lag, count, sum, 
    avg, hour, array, array_contains, array_distinct, collect_list, 
    array_compact, when, lit, least, max as max_func
)
from pyspark.sql.types import LongType, DoubleType
from delta.tables import DeltaTable

print("Starting consolidated pipeline run...")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Bronze Ingestion Layer
...
print(f"Bronze Ingestion complete in {time.time() - step_start:.2f} seconds.")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Silver Processing Layer
...
def process_silver_batch(batch_df, batch_id):
    ...
    max_time_df = enriched_df.select(spark_max("transaction_time").alias("max_time"))
    ...
    (silver_table.alias("target")
         .merge(valid_df.alias("source"), "target.transaction_id = source.transaction_id")
...
print(f"Silver Processing complete in {time.time() - step_start:.2f} seconds.")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Gold Fraud Detection & Aggregation Layer

# COMMAND ----------

step_start = time.time()
print("Starting Gold Fraud Detection...")
...
def process_gold_batch(batch_df, batch_id):
    ...
    ).withColumn("fraud_rules_triggered", array_compact(col("rules_list"))).drop("rules_list")
    ...
    if not alerts_df.isEmpty():
        ...
        (alerts_table.alias("target")
             .merge(alerts_df.alias("source"), "target.transaction_id = source.transaction_id")
             .whenMatchedUpdateAll()
             .whenNotMatchedInsertAll()
             .execute())

    # 2. Merge 24-hour windowed customer risk profiles
    ...
    (profile_table.alias("target")
         .merge(profile_df.alias("source"), "target.customer_id = source.customer_id")
         .whenMatchedUpdateAll()
         .whenNotMatchedInsertAll()
         .execute())
...
print(f"Gold Fraud Detection complete in {time.time() - step_start:.2f} seconds.")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Optimize Gold Tables (ZORDER)
...
if RUN_MAINTENANCE_OPTIMIZATION:
    ...
    print(f"Z-Ordering complete in {time.time() - step_start:.2f} seconds.")
...
print(f"Total Data Processing Duration: {total_elapsed:.2f} seconds")
print(f"Performance Status: {'MET (< 60s)' if total_elapsed < 60 else 'EXCEEDS 60s (Cold Start)'}")
